...
let update (d:Dictionary<Cave, list<Cave>>) s1 s2 =
    if d.ContainsKey s1 then d[ s1 ] <- s2 :: d[ s1 ]
    else d.Add(s1, [ s2 ])
    
let d = Dictionary<Cave, Cave list>()
...
let rec recurser1 (d:Dictionary<Cave, list<Cave>>) history key =
     if key = End then 1
     else
...
let rec recurser2 (d:Dictionary<Cave, list<Cave>>) repeatAllowed history key =
    if key = End then 1
    else
