...
module Day16 =
    ...
    and Expr = Lit of int64 | Op of Type_ * Packet seq
    ...
    let int64FromBase (base_: int) s = Convert.ToInt64(s.ToString(), base_)    
    ...
    let (|LitType|OpType|) = function | 4 -> LitType | _ -> OpType
    ...
    let rec toPacket str =
        ...
        | LitType ->
            ...
            { Version = version; Payload = Lit literal }, remainder
            
        | OpType ->
            let subPackets, remainder = toLimit (Seq.head rest) |> toSubPackets (Seq.tail rest)
            
            { Version = version; Payload = Op (type_, subPackets) }, remainder
    ...
    and toSubPackets str limit  =
        let limitBits, rest = splitString (limitToSize limit) str
        let total = intFromBase 2 limitBits
        let packetsRaw = Seq.unfold ((limitToFunc limit) total) (0, rest)
    ...
    let rec sumVersions = function
        | { Version = v; Payload = Lit _ } -> v
        | { Version = v; Payload = Op (_, packets) } -> v + Seq.sumBy sumVersions packets
    
    // PART 2
    
    // collapses a packet into an integer based on the expressions for each operation
    let rec collapse = function
        | { Payload = Lit value } -> value
        | { Payload = Op (type_, packets) } ->
            match type_ with
...
module Day16test =
    ...
    let ``Given a string of hex numbers, packets produce the valid version sum`` (input, expected) =
        (hexesToBinary >> toPacket >> fst >> sumVersions) input
